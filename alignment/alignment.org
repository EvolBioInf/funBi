...
identify these mutations. In this exercise we show to calculate
...
are free to ignore. We rename the mutated sequence Rand3 and align it
...
#+end_src
#+begin_export latex
The two sequences now contain 5 mismatches, and the alignment score
has decreased to 80. Notice that mismatches lack the match mark, `|'.
\scriptsize \verbatiminput{../alignment/al2.txt} \normalsize
...
sequence. Then we align \ty{seq1.fasta} to \ty{seq4.fasta} with
\ty{al}.
#+end_export
#+begin_src sh <<seq.sh>>=
...
not distinguished and instead are collectively called `indels'.
\scriptsize \verbatiminput{../alignment/al3.txt} \normalsize
