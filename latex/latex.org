...
#+end_src
#+begin_export latex
For the actual picture we first draw a grid of ``help lines''. This
makes it easier to locate coordinates for drawing and can be removed
in the final picture. Notice the semicolon at the end of the draw
command
#+end_export
...
6 and save them to the file \ty{r.dat}. The modulo operator in
\verb+a % b+, divides \ty{a} by \ty{b} and returns the remainder. So
our expression \verb!$(($RANDOM % 6 + 1))! takes a random number,
