...
shall use. Create the file \ty{hello.tex} in \ty{emacs}. The file's
...
window containing pdf is closed again; not a big deal, just a bit
inconvenient.
#+end_export
#+begin_src sh <<cli>>=
  evince hello.pdf &
...
program''~\cite{kol24:unl}. So we add to the header the command for
...
dice 100 times. That sounds like a lot, but don't worry, we leave the
...
divides it by 6, and ads one to the remainder. This simulates the
...
we add a plot of the data we just generated. Figure~\ref{fig:data}
shows our first data plot. Since we are plotting random numbers, yours
is bound to look different.
\begin{figure}
  \begin{center}
    \input{../latex/tikz_2}
  \end{center}
  \caption{Our first data plot showing the results of 100 dice rolls
    simulated on the computer.}\label{fig:data}
\end{figure}
#+end_export
#+begin_src latex <<Body, File \ref{fil:hel}>>=
  \begin{tikzpicture}
